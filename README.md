...
- 상단 검색 → 노선 추가. **칩 탭 = 그 노선을 지도에서 껐다 켜기**(`enabled`), `×` 는 삭제.
...
- **정류장 탭** → 도착예정 순 목록을 `내 노선`(등록해 둔 것) / `그 외 노선` 두 그룹으로.
  `이 정류장 지나는 노선만 보기` 로 지도를 **기준 정류장** 경유 노선만 남길 수 있다
  (판정은 각 노선의 `stops` arsId 목록 — 운행 여부와 무관. 추적 중인 노선은 필터에서 제외되지 않음).
  필터 대상은 항상 기준 정류장이라 상태는 불리언 하나(`onlyRef`) — 기준이 바뀌면 필터도 따라가므로
  도보시간·확률 계산이 다른 정류장을 가리킬 일이 없다.
- 필터가 켜져 있는 동안, 그 정류장에 **아직 도착하지 않은 버스 중 가장 가까운 1대**의 지도 마커에
  `3분 후 · 탈 확률 82%` 배지를 붙인다. 여러 대에 붙이면 정작 탈 버스가 묻히므로 딱 하나만.
- 기준 정류장이 잡혀 있으면 하단에 카드가 남는다. **탭하면 그 정류장으로 지도를 옮기고**
  도착 정보를 연다(지도 아이콘 탭과 같은 경로).
